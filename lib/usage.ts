...
const FREE_COMPARABLES_LIMIT = 5
...
export async function initializeMonthlyUsage(userId: string): Promise<UsageTracking> {
  const supabase = await createClient()
  const now = new Date()
  const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
...
export function getComparablesLimit(): number {
  return FREE_COMPARABLES_LIMIT
}
